...
async function run(): Promise<void> {
  try {
    ...
    if (!('nx' in packageObject.dependencies)) {
      core.setFailed('NX package can not be detected. Make sure you provided the correct package.json file and NX is installed.')
      return
    }

    const currentNxVersion = packageObject.dependencies['dependency-name'].replace(/[\^~]/, '')
    ...
    await exec('git add .')
    await exec(`git  commit -m ${commitMessage.replace("$VERSION", latestNxVersion)}`);
    await exec('git push')
